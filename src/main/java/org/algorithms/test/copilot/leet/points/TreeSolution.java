package org.algorithms.test.copilot.leet.points;
...
class TreeSolution {
    public int[] maxPoints(int[][] grid, int[] queries) {
        ...
        Arrays.sort(queries); // Sort queries for sequential processing

        for (int qIdx = 0; qIdx < queries.length; qIdx++) {
            int queryValue = queries[qIdx];
            ...
            result[qIdx] = points; // Store result for current query
        }
